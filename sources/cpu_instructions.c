...
/*   Updated: 2018/04/05 19:31:29 by upopee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		live_instr(t_vcpu *c)			// NEEDS TO BE COMPLETED
{
	uint32_t	player_no;

	secure_fetch(c->pc[0] + OPBC_SIZE, c->memory, c->op_args, ARG_DIRSZ);
	player_no = c->op_args[0];
...
int		ld_instr(t_vcpu *c)
{
	...
	c->carry[0] = 0;
	if ((reg_dst = c->op_args[1]) != 0 && --reg_dst < REG_NUMBER)
	{
		data = c->op_args[0];
		r = c->registers;
		if (((c->op_bytecode >> 6) & 0x03) == ARG_IND)
		{
			secure_fetch(data, c->memory, r + reg_dst, REG_SIZE);
		...
		c->carry[0] = (r[reg_dst] == 0);
	}
	else
		log_this("ins", 0, LD_KO, c->op_args[1]);
	return (0);
}

int		st_instr(t_vcpu *c)
{
	uint32_t	*r;
	uint32_t	data;
	uint8_t		reg_src;

	if ((reg_src = c->op_args[0]) != 0 || --reg_src < REG_NUMBER)
	{
		r = c->registers;
		data = c->op_args[1];
		if (((c->op_bytecode >> 4) & 0x03) == ARG_IND)
		{
			secure_store(data, c->memory, r[reg_src], REG_SIZE);
		...
		else if (data != 0 && --data < REG_NUMBER)
		{
			...
			log_this("ins", 0, ST_KO, c->op_args[1]);
	}
	else
		log_this("ins", 0, ST_KO, c->op_args[0]);
	return (0);
}

int		add_instr(t_vcpu *c)
{
	uint32_t	*r;
	uint8_t		reg_a;
	uint8_t		reg_b;
	...
	c->carry[0] = 0;
	if ((reg_dst = c->op_args[2]) != 0 && --reg_dst < REG_NUMBER
		&& (reg_a = c->op_args[0]) != 0 && --reg_a < REG_NUMBER
		&& (reg_b = c->op_args[1]) != 0 && --reg_b < REG_NUMBER)
	{
		r = c->registers;
...
int		and_instr(t_vcpu *c)
{
	...
	{
		r = c->registers;
		if (decode_arg(c, (c->op_bytecode >> 6) & 0x03, arg) == FAILURE)
			return (log_this("ins", 0, AND_KO, arg[0]));
		if (decode_arg(c, (c->op_bytecode >> 4) & 0x03, arg + 1) == FAILURE)
...
int		zjmp_instr(t_vcpu *c)
{
	int16_t		restricted;

	if (c->carry[0] == 1)
	{
		secure_fetch(c->pc[0] + OPBC_SIZE, c->memory, c->op_args, ARG_INDSZ);
		restricted = (int16_t)(c->op_args[0] & 0xFFFF) % IDX_MOD;
		c->pc[0] = jump_to(c->pc[0], restricted);
		log_this("ins", 0, P_ARG_IND, 1, (int16_t)(c->op_args[0] & 0xFFFF));
		log_this("ins", 0, P_IND_MOD, restricted);
		log_this("ins", 0, ZJMP_OK, restricted, c->pc[0]);
		return (-OPBC_SIZE);
	}
	else
	{
...
int		ldi_instr(t_vcpu *c)
{
	...
	if ((reg_dst = arg[2]) != 0 && --reg_dst < REG_NUMBER)
	{
		...
		data = jump_to(c->pc[0], (int)data);
		secure_fetch(data, c->memory, c->registers + reg_dst, REG_SIZE);
		c->carry[0] = (c->registers[reg_dst] == 0);
		log_this("ins", 0, LDI_OK, arg[0], arg[1],
						data, reg_dst + 1, c->registers[reg_dst]);
	}
	else
...
int		sti_instr(t_vcpu *c)
{
	...
	if ((reg_src = arg[0]) != 0 && --reg_src < REG_NUMBER)
	{
		if (decode_arg(c, (c->op_bytecode >> 4) & 0x03, arg + 1) == FAILURE)
			return (log_this("ins", 0, LDI_KO, arg[1]));
		if (decode_arg(c, (c->op_bytecode >> 2) & 0x03, arg + 2) == FAILURE)
			return (log_this("ins", 0, LDI_KO, arg[2]));
		data = ((int)arg[1] + (int)arg[2]) % IDX_MOD;
		data = jump_to(c->pc[0], (int)data);
		secure_store(data, c->memory, c->registers[reg_src], REG_SIZE);
		log_this("ins", 0, STI_OK, reg_src + 1, arg[1],
					arg[2], data, c->registers[reg_src]);
	}
	else
...
int		lld_instr(t_vcpu *c)
{
	...
	if ((reg_dst = c->op_args[1]) != 0 && --reg_dst < REG_NUMBER)
	{
		...
		log_this("ins", 0, LLD_KO, c->op_args[1]);
	return (0);
}

int		lldi_instr(t_vcpu *c)
{
	uint32_t	*arg;
	uint32_t	data;
	uint8_t		reg_dst;
...
int		aff_instr(t_vcpu *c)
{
	...
	if ((reg_src = c->op_args[0]) != 0 && --reg_src < REG_NUMBER)
	{
		to_print = c->registers[reg_src] & 0xFF;
		write(1, &to_print, 1);
		log_this("ins", 0, AFF_OK, to_print, reg_src + 1);
	}
	else
		log_this("ins", 0, AFF_KO, c->op_args[0]);
	return (0);
}
