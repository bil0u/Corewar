...
/*   Updated: 2018/04/23 19:11:18 by upopee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ld_instr(t_vcpu *cpu, t_process *p)
{
	...
	if (((CPU_OPBC >> 6) & 3) == ARG_IND)
	{
		...
		p->carry = (r[r_dst - 1] == 0);
		INS_DEB ? log_this(IDW, D_LDIND, IDA,
	...
	else
	{
		r[r_dst - 1] = data;
		p->carry = (r[r_dst - 1] == 0);
		INS_DEB ? log_this(IDW, D_LDDIR, IDA, data, r_dst, p->carry) : 0;
		INS_VERB ? ft_printf(V_LD, IVA, data, r_dst) : 0;
	}
	return (0);
}
