...
/*   Updated: 2018/04/19 01:00:46 by upopee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		live_instr(t_vcpu *cpu, t_process *p, t_player *pl, t_gamectrl *game)
{
	...
	if (p_alive > 0 && p_alive < MAX_PLAYERS + 1)
	{
		...
		++game->nb_lives;
		log_this("ins", 0, LIVE_OK, cpu->tick, p->player_no, p->pid, p_alive);
	}
	else
		log_this("ins", 0, LIVE_KO, cpu->tick, p->player_no, p->pid, p_alive);
	return (ARG_DIRSZ);
}
...
int		ld_instr(t_vcpu *cpu, t_process *p)
{
	...
	if ((reg_dst = cpu->op_args[1]) != 0 && --reg_dst < REG_NUMBER)
	{
		...
		if (((cpu->op_bytecode >> 6) & 0x03) == ARG_IND)
		{
			...
			log_this("ins", 0, LD_IND, cpu->tick, p->player_no, p->pid, data, r[reg_dst], reg_dst + 1);
		}
		else
		{
			r[reg_dst] = data;
			log_this("ins", 0, LD_DIR, cpu->tick, p->player_no, p->pid, data, reg_dst + 1);
		}
		p->carry = (r[reg_dst] == 0);
	}
	else
		log_this("ins", 0, LD_KO, cpu->tick, p->player_no, p->pid, cpu->op_args[1]);
	return (0);
}
...
int		st_instr(t_vcpu *cpu, t_process *p)
{
	...
	if ((reg_src = cpu->op_args[0]) != 0 || --reg_src < REG_NUMBER)
	{
		...
		if (((cpu->op_bytecode >> 4) & 0x03) == ARG_IND)
		{
			...
			log_this("ins", 0, ST_IND, cpu->tick, p->player_no, p->pid, reg_src + 1, r[reg_src], data);
		}
		else if (data != 0 && --data < REG_NUMBER)
		{
			...
			log_this("ins", 0, ST_KO, cpu->tick, p->player_no, p->pid, cpu->op_args[1]);
	}
	else
		log_this("ins", 0, ST_KO, cpu->tick, p->player_no, p->pid, cpu->op_args[0]);
	return (0);
}
...
int		add_instr(t_vcpu *cpu, t_process *p)
{
		...
		&& (reg_b = cpu->op_args[1]) != 0 && --reg_b < REG_NUMBER)
	{
		...
		p->carry = (r[reg_dst] == 0);
		log_this("ins", 0, ADD_OK, cpu->tick, p->player_no, p->pid, ++reg_a, ++reg_b, r[reg_dst], reg_dst + 1);
	}
	else
...
int		sub_instr(t_vcpu *cpu, t_process *p)
{
		...
		&& (reg_b = cpu->op_args[1]) != 0 && --reg_b < REG_NUMBER)
	{
		...
		r[reg_dst] = (uint32_t)((int)(r[reg_a]) - (int)(r[reg_b]));
		p->carry = (r[reg_dst] == 0);
		log_this("ins", 0, SUB_OK, cpu->tick, p->player_no, p->pid, ++reg_a, ++reg_b, r[reg_dst], reg_dst + 1);
	}
	else
...
int		and_instr(t_vcpu *cpu, t_process *p)
{
	...
	if ((reg_dst = arg[2]) != 0 && --reg_dst < REG_NUMBER)
	{
		...
		if (decode_arg(cpu, p, (cpu->op_bytecode >> 6) & 0x03, arg) == FAILURE)
			return (log_this("ins", 0, AND_KO, cpu->tick, p->player_no, p->pid, arg[0]));
		if (decode_arg(cpu, p, (cpu->op_bytecode >> 4) & 0x03, arg + 1) == FAILURE)
			return (log_this("ins", 0, AND_KO, cpu->tick, p->player_no, p->pid, arg[1]));
		r[reg_dst] = arg[0] & arg[1];
		p->carry = (r[reg_dst] == 0);
		log_this("ins", 0, AND_OK, cpu->tick, p->player_no, p->pid, arg[0], arg[1], r[reg_dst], reg_dst + 1);
	}
	else
		log_this("ins", 0, AND_KO, cpu->tick, p->player_no, p->pid, arg[2]);
	return (0);
}
...
int		or_instr(t_vcpu *cpu, t_process *p)
{
	...
	if ((reg_dst = arg[2]) != 0 && --reg_dst < REG_NUMBER)
	{
		r = p->registers;
		if (decode_arg(cpu, p, (cpu->op_bytecode >> 6) & 0x03, arg) == FAILURE)
			return (log_this("ins", 0, OR_KO, cpu->tick, p->player_no, p->pid, arg[0]));
		if (decode_arg(cpu, p, (cpu->op_bytecode >> 4) & 0x03, arg + 1) == FAILURE)
			return (log_this("ins", 0, OR_KO, cpu->tick, p->player_no, p->pid, arg[1]));
		r[reg_dst] = arg[0] | arg[1];
		p->carry = (r[reg_dst] == 0);
		log_this("ins", 0, OR_OK, cpu->tick, p->player_no, p->pid, arg[0], arg[1], r[reg_dst], reg_dst + 1);
	}
	else
		log_this("ins", 0, OR_KO, cpu->tick, p->player_no, p->pid, arg[2]);
	return (0);
}
...
int		xor_instr(t_vcpu *cpu, t_process *p)
{
	...
	if ((reg_dst = arg[2]) != 0 && --reg_dst < REG_NUMBER)
	{
		r = p->registers;
		if (decode_arg(cpu, p, (cpu->op_bytecode >> 6) & 0x03, arg) == FAILURE)
			return (log_this("ins", 0, XOR_KO, cpu->tick, p->player_no, p->pid, arg[0]));
		if (decode_arg(cpu, p, (cpu->op_bytecode >> 4) & 0x03, arg + 1) == FAILURE)
			return (log_this("ins", 0, XOR_KO, cpu->tick, p->player_no, p->pid, arg[1]));
		r[reg_dst] = arg[0] ^ arg[1];
		p->carry = (r[reg_dst] == 0);
		log_this("ins", 0, XOR_OK, cpu->tick, p->player_no, p->pid, arg[0], arg[1], r[reg_dst], reg_dst + 1);
	}
	else
		log_this("ins", 0, XOR_KO, cpu->tick, p->player_no, p->pid, arg[2]);
	return (0);
}
...
int		ldi_instr(t_vcpu *cpu, t_process *p)
{
	...
	{
		if (decode_arg(cpu, p, (cpu->op_bytecode >> 6) & 0x03, arg) == FAILURE)
			return (log_this("ins", 0, LDI_KO, cpu->tick, p->player_no, p->pid, arg[0]));
		if (decode_arg(cpu, p, (cpu->op_bytecode >> 4) & 0x03, arg + 1) == FAILURE)
			return (log_this("ins", 0, LDI_KO, cpu->tick, p->player_no, p->pid, arg[1]));
		...
		log_this("ins", 0, LDI_OK, cpu->tick, p->player_no, p->pid, arg[0], arg[1],
...
int		sti_instr(t_vcpu *cpu, t_process *p)
{
	...
	if ((reg_src = arg[0]) != 0 && --reg_src < REG_NUMBER)
	{
		...
		if (decode_arg(cpu, p, (cpu->op_bytecode >> 2) & 0x03, arg + 2) == FAILURE)
			return (log_this("ins", 0, LDI_KO, cpu->tick, p->player_no, p->pid, arg[2]));
		data = ((int)arg[1] + (int)arg[2]) % IDX_MOD;
		data = jump_to(p->pc, (int)data);
		secure_store(data, cpu->memory, p->registers[reg_src], REG_SIZE);
		log_this("ins", 0, STI_OK, cpu->tick, p->player_no, p->pid, reg_src + 1, arg[1],
...
int		fork_instr(t_vcpu *cpu, t_process *p, t_player *pl)
{
	...
	log_this("ins", 0, FORK_OK, cpu->tick, p->player_no, p->pid, child.pid, p->pc,
...
int		lld_instr(t_vcpu *cpu, t_process *p)
{
	...
	if ((reg_dst = cpu->op_args[1]) != 0 && --reg_dst < REG_NUMBER)
	{
		...
		{
			secure_fetch(data, cpu->memory, r + reg_dst, REG_SIZE);
			log_this("ins", 0, LLD_IND, cpu->tick, p->player_no, p->pid, data, reg_dst + 1, r[reg_dst]);
			ft_printf("%d\n", reg_dst);
		}
		else
		{
			r[reg_dst] = data;
			log_this("ins", 0, LLD_DIR, cpu->tick, p->player_no, p->pid, data, reg_dst + 1);
		}
		p->carry = (r[reg_dst] == 0);
	}
	else
		log_this("ins", 0, LLD_KO, cpu->tick, p->player_no, p->pid, cpu->op_args[1]);
	return (0);
}
...
int		lldi_instr(t_vcpu *cpu, t_process *p)
{
	...
	{
		if (decode_arg(cpu, p, (cpu->op_bytecode >> 6) & 0x03, arg) == FAILURE)
			return (log_this("ins", 0, LLDI_KO, cpu->tick, p->player_no, p->pid, arg[0]));
		if (decode_arg(cpu, p, (cpu->op_bytecode >> 4) & 0x03, arg + 1) == FAILURE)
			return (log_this("ins", 0, LLDI_KO, cpu->tick, p->player_no, p->pid, arg[1]));
		data = ((int)arg[0] + (int)arg[1]);
		data = jump_to(p->pc, (int)data);
		secure_fetch(data, cpu->memory, p->registers + reg_dst, REG_SIZE);
		p->carry = (p->registers[reg_dst] == 0);
		log_this("ins", 0, LLDI_OK, cpu->tick, p->player_no, p->pid, arg[0], arg[1],
						data, reg_dst + 1, p->registers[reg_dst]);
	}
	else
		log_this("ins", 0, LLDI_KO, cpu->tick, p->player_no, p->pid, arg[2]);
	return (0);
}
...
int		lfork_instr(t_vcpu *cpu, t_process *p, t_player *pl)
{
	...
	cpu->ctrl->sleep_time = 1000000 /
		(cpu->ctrl->cycles_sec * cpu->jobs->nb_processes);
	log_this("ins", 0, LFORK_OK, cpu->tick, p->player_no, p->pid, child.pid, p->pc,
								(int32_t)cpu->op_args[0], child.pc);
	return (ARG_INDSZ);
}
...
int		aff_instr(t_vcpu *cpu, t_process *p, t_player *pl)
{
	...
	if ((reg_src = cpu->op_args[0]) != 0 && --reg_src < REG_NUMBER)
	{
		...
		log_this("ins", 0, AFF_OK, cpu->tick, p->player_no, p->pid, to_print, reg_src + 1);
	}
	else
		log_this("ins", 0, AFF_KO, cpu->tick, p->player_no, p->pid, cpu->op_args[0]);
	return (0);
}
